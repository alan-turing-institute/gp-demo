...
<script lang="ts">
	...
	import ShowBivariateCovarianceMatrix from '$lib/ShowBivariateCovarianceMatrix.svelte';
	import PlotMarginals from '$lib/PlotMarginals.svelte';
	import PlotKernelSlices from '$lib/PlotKernelSlices.svelte';
	import PlotBivariateCovariance from '$lib/PlotBivariateCovariance.svelte';
	import RandomSample from '$lib/RandomSample.svelte';
	import Animation from '$lib/Animation.svelte';
	import ConfigModel from '$lib/ConfigModel.svelte';
	import ConfigPlot from '$lib/ConfigPlot.svelte';
	...
	import { linspace, matrixSqrt, covEllipse } from '$lib/mymath';
	import { getIndicesAndFrac } from '$lib/binarysearch';
	import type { IndicesAndFrac } from '$lib/binarysearch';
	import { posterior, prior } from '$lib/gpposterior';

	// variables for ConfigModel
	let { choices: kernelChoices, selected: kernelSelection } = createKernelChoices();
	let { choices: kernelChoices2, selected: kernelSelection2 } = createKernelChoices();
...
<div>
	...
	<p>
		<em
			>Works best in Chrome, works fine in Firefox; not tested in Safari/Edge. Touchscreen
			interactions supported, but works best on desktop.</em
		>
	</p>
	...
	<CollapsibleCard open={false}>
		...
		<div slot="body">
			A Gaussian process can be thought of as an extension of the multivariate normal distribution
			to an infinite number of random variables covering each point on the input domain. The
			covariance between function values at any two points is given by the evaluation of the <em
				>kernel</em
			>
			of the Gaussian process. For an in-depth explanation, read this
	...
	<CollapsibleCard open={false}>
		...
		<div slot="body" class="text-container">
			<div class="text-explanation" style="grid-area: marginals;">
				<em>Bottom left:</em>
				Visualises the Gaussian process <Katex math="f(\cdot)" /> through its mean (central
				<span style="color: rgb(0, 100, 100);">d-a-s-h-e-d</span>
				line) and
				<Katex math="\pm \sigma" /> and <Katex math="\pm 2 \sigma" /> confidence bands (<span
					style="background-color: rgb(0, 100, 100, 0.2);"
					>sha<span style="background-color: rgb(0, 100, 100, 0.2);">ded ar</span>eas</span
				>). Samples from the Gaussian process are drawn in colored lines. The marginal distributions
				of <Katex math="f(x_1)" /> and <Katex math="f(x_2)" /> are plotted at the vertical lines at <Katex
					math="x_1"
				/> (<span style="color: red;">red</span>) and <Katex math="x_2" /> (<span
					style="color: orange;">orange</span
				>). You can change their location by <strong>moving the mouse</strong>
				with or without holding the <strong>Shift key</strong>. You can add observations on which to
				condition the Gaussian process by
				<strong>clicking</strong>
				anywhere in the plot; these observations are drawn as black circles &#9899;. To remove an observation,
				click on it again.
				<small
					><em>Note:</em> Two observations too close to each other can lead to numerical issues and long
					compute times. If the app seems to hang, reload the page to restart.</small
				>
			</div>
			<div class="text-explanation" style="grid-area: kernelslices;">
				<em>Top left:</em>
				Visualises a slice through the covariance function or kernel <Katex math="k(x_i, \cdot)" /> as
				a function of the second argument, for <Katex math="x_i=x_1" />
				...
				/>, and <Katex math="k(x_2, x_2)" />. You can change the location of <Katex math="x_1" /> and
				<Katex math="x_2" /> by <strong>moving the mouse</strong> with or without holding the
			...
			<div class="text-explanation" style="grid-area: bicovmatrix">
				...
				/>, corresponding to the circles in the top-left plot, and shows the correlation
				coefficient.
			</div>
			<div class="text-explanation" style="grid-area: bicovplot;">
				<em>Bottom right:</em>
				Visualises the covariance between <Katex math="f(x_1)" /> and <Katex math="f(x_2)" />
				(<span style="background-color: rgb(0, 100, 100, 0.2);"
					>sha<span style="background-color: rgb(0, 100, 100, 0.2);">ded ar</span>eas</span
				>) and the samples evaluated at those points (colored circles, corresponding to the colored
				lines in the bottom-left plot) as well as the marginal distributions of <Katex
					math="f(x_1)"
				/>
